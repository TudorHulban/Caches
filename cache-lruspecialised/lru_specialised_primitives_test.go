...
func TestPrimitives_LRU(t *testing.T) {
	cache := NewCacheLRU[int, int](
		&ParamsNewCacheLRU{
			Capacity: 10,
		},
	)
...
func TestLRU_TimeCapacity1(t *testing.T) {
	ttlCache := 300 * time.Millisecond
	...
	cache.PutTTL(key, value)

	reconstructed1, errGet1 := cache.Get(key)
	require.NoError(t, errGet1)
	require.Equal(t,
		value,
		*reconstructed1,
	...
	reconstructed2, errGet2 := cache.Get(key)
	require.Error(t, errGet2)
	require.Nil(t, reconstructed2)
}
